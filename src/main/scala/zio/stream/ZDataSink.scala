...
/** Sinks to interoperate with JDK's [[java.io.DataInput]] and [[java.io.DataOutput]] format in a way analogous to JDK's
  * [[java.io.DataInputStream]].
  *
  * Note: The JDK's interfaces define methods that operate on mutable data (`Byte` arrays passed in as arguments). ZIO
  * Streams and Sinks are meant for immutable data and use [[zio.Chunk]] instead.
  *
  * Generally, Sinks are provided for each method of the [[java.io.DataInput]] interface, with these exceptions:
  *
  *   - `void readFully(byte[] b)`: Obtain a `Chunk[Byte]` via `ZSink.take(n)` (where `n` is `b.length`) instead
  *   - `void readFully(byte[] b, int off, int len)`: Obtain a `Chunk[Byte]` via `ZSink.take(len)` instead
  *
  * And this one under a different name:
  *
  *   - `String readUTF()`: Renamed `readModifiedUTF8`
  *
  * There are an additional public utility Sinks for reading lengths and Chunks not present in [[java.io.DataInput]]:
  *
  *   - `readIntLength` that reads an `Int` length and ensures it is non-negative.
  *   - `readIntLengthChunk` that reads an `Int` length and ensures it is non-negative.
  *   - `readN` that reads using a given Sink `n` times, producing a Chunk as output.
  *   - `readUnsignedShortLength` that reads a two-Byte `Int` length and ensures it is non-negative.
  *   - `readUnsignedShortLengthChunk` that reads a two-Byte length and then reads and returns that many Bytes in a
  *     Chunk
  *   - `readUntil` that reads using a given Sink until the result matches a predicate, omitting final read result
  *
  * To do's:
  *   - TODO: readLine()
  *   - TODO: skipBytes() -- Is there a version of ZSink.drop() that we could use?
  *   - TODO: Use EOFException as error type everywhere, or... what? ZSink.foldWhile can error out due to EOF but does
  *     not declare that in its type...
  *
  * @see
...
object ZDataSink {
  ...
  /** Consumes one `Byte` and succeeds with `false` if zero, or `true` if non-zero. Fails with [[java.io.EOFException]]
    * if unable to consume a `Byte`.
    */
  ...
  /** Consumes one `Byte` and succeeds with its value. Fails with [[java.io.EOFException]] if unable to consume a
    * `Byte`.
    */
  ...
  /** Consumes two `Byte`s and succeeds with their (big-endian) `Char` value. Fails with [[java.io.EOFException]] if < 2
    * `Byte`s are available.
    */
  def readChar: ZSink[Any, Nothing, Byte, Byte, Char] =
    readUnsignedShort.map(_.toChar)

  /** Consumes eight `Byte`s and succeeds with their (big-endian) `Long` value converted by
    * [[java.lang.Double.longBitsToDouble]]. Fails with[[java.io.EOFException]] if < 8 `Byte`s are available.
    */
  def readDouble: ZSink[Any, Nothing, Byte, Byte, Double] =
    readLong.map(java.lang.Double.longBitsToDouble)

  /** Consumes four `Byte`s and succeeds with their (big-endian) `Int` value converted by
    * [[java.lang.Float.intBitsToFloat]]. Fails with [[java.io.EOFException]] if < 4 `Byte`s are available.
    */
  def readFloat: ZSink[Any, Nothing, Byte, Byte, Float] =
    readInt.map(java.lang.Float.intBitsToFloat)

  /** Consumes four `Byte`s and succeeds with their (big-endian) `Int` value. Fails with [[java.io.EOFException]] if < 4
    * `Byte`s are available.
    */
  def readInt: ZSink[Any, Nothing, Byte, Byte, Int] =
    ZSink.foldUntil[Byte, Int](0, 4) { (v, b) => (v << 8) + (b & 255) }

  /** Consumes four `Byte`s and succeeds with their (big-endian) `Int` value, or fails with [[NegativeLengthException]]
    * if the value is negative. Fails with [[java.io.EOFException]] if < 4 `Byte`s are available.
    */
  ...
  /** Consumes eight `Byte`s and succeeds with their (big-endian) `Long` value. Fails with [[java.io.EOFException]] if <
    * 8 `Byte`s are available.
    */
  def readLong: ZSink[Any, Nothing, Byte, Byte, Long] =
    ZSink.foldUntil[Byte, Long](0L, 8) { (v, b) => (v << 8) + (b & 255).toLong }

  /** Reads via given Sink an integer number of times, returning a Chunk of values read. */
  def readN[A, E](
      n: Int,
  ...
  /** Reads via given Sink until the resulting value matches a predicate, returning a Chunk of values read (not
  ...
  /** Consumes two `Byte`s and succeeds with their (big-endian) `Short` value. Fails with [[java.io.EOFException]] if <
    * 2 `Byte`s are available.
    */
  def readShort: ZSink[Any, Nothing, Byte, Byte, Short] =
    readUnsignedShort.map(_.toShort)

  /** Consumes one `Byte` and succeeds with its unsigned value as an Int. Fails with [[java.io.EOFException]] if unable
    * to consume a `Byte`.
    */
  ...
  /** Consumes two `Byte`s and succeeds with their (big-endian) `Short` unsigned value as an Int. Fails with
    * [[java.io.EOFException]] if < 2 `Byte`s are available.
    */
